...
public class KISA_SEED_CBC {
	...
	private static void RoundKeyUpdate0(int T[], int[] K, int K_offset, int ABCD[], int KC) {
		...
		K[K_offset] = SS0[GetB0(T[0]) & 0x0ff] ^ SS1[GetB1(T[0]) & 0x0ff] ^ SS2[GetB2(T[0]) & 0x0ff] ^ SS3[GetB3(T[0]) & 0x0ff];
	...
	private static void BLOCK_XOR_CBC(int[] OUT_VALUE, int out_value_offset, int[] IN_VALUE1, int in_value1_offset,
		...
		OUT_VALUE[out_value_offset] = ((in_value1_offset < IN_VALUE1.length) ? IN_VALUE1[in_value1_offset] : 0) ^ ((in_value2_offset < IN_VALUE2.length) ? IN_VALUE2[in_value2_offset] : 0);
	...
	private static void KISA_SEED_Encrypt_Block_forCBC(int[] in, int in_offset, int[] out, int out_offset,
													   KISA_SEED_CBC.KISA_SEED_KEY ks) {
		...
		if(KISA_SEED_CBC.Common.BIG_ENDIAN != ENDIAN) {
	...
	public static int[] chartoint32_for_SEED_CBC(byte[] in, int inLen) {
		...
		for(i = 0; i < len; i++) {
			KISA_SEED_CBC.Common.byte_to_int(data, i, in, i * 4, ENDIAN);
		}
	...
	public static byte[] int32tochar_for_SEED_CBC(int in[], int inLen) {
		...
		data = new byte[inLen];
		if(ENDIAN != KISA_SEED_CBC.Common.BIG_ENDIAN) {
			for(i = 0; i < inLen; i++) {
	...
	public static void SEED_CBC_init(KISA_SEED_CBC.KISA_SEED_INFO pInfo, KISA_SEED_CBC.KISA_ENC_DEC enc,
									 byte[] pbszUserKey, byte[] pbszIV) {
		SeedRoundKey(pInfo, enc, pbszUserKey, pbszIV);
	}
	
	
	public static void SEED_CBC_Process(KISA_SEED_CBC.KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out,
										int[] outLen) {
		...
		if(KISA_SEED_CBC.KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			pdwXOR = pInfo.ivec;
			...
			KISA_SEED_CBC.Common.memcpy(pInfo.ivec, pdwXOR, pdwXOR_offset, BLOCK_SIZE_SEED);
			KISA_SEED_CBC.Common.memcpy(pInfo.cbc_buffer, in, in_offset, pInfo.buffer_length);
		} else {
			pdwXOR = pInfo.ivec;
			...
			KISA_SEED_CBC.Common.memcpy(pInfo.ivec, pdwXOR, pdwXOR_offset, BLOCK_SIZE_SEED);
			KISA_SEED_CBC.Common.memcpy(pInfo.cbc_last_block, out, out_offset - BLOCK_SIZE_SEED_INT, BLOCK_SIZE_SEED);
	...
	public static int SEED_CBC_Close(KISA_SEED_CBC.KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen) {
		...
		if(KISA_SEED_CBC.KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			for(i = pInfo.buffer_length; i < BLOCK_SIZE_SEED; i++) {
				KISA_SEED_CBC.Common.set_byte_for_int(pInfo.cbc_buffer, i, (byte) nPaddngLeng, ENDIAN);
			}
			...
			nPaddngLeng = KISA_SEED_CBC.Common.get_byte_for_int(pInfo.cbc_last_block, BLOCK_SIZE_SEED - 1, ENDIAN);
			
			
			if((nPaddngLeng > 0) && (nPaddngLeng <= BLOCK_SIZE_SEED)) {
				for(i = nPaddngLeng; i > 0; i--) {
					KISA_SEED_CBC.Common.set_byte_for_int(out, out_offset - i, (byte) 0x00, ENDIAN);
				}
	...
	public static byte[] SEED_CBC_Encrypt(byte[] pbszUserKey, byte[] pbszIV, byte[] message, int message_offset,
		...
		KISA_SEED_CBC.Common.arraycopy(newpbszPlainText, pbszPlainText, nPlainTextLen);
		
		byte[] pbszCipherText = new byte[newpbszPlainText.length];
		
		
		SEED_CBC_init(info, KISA_SEED_CBC.KISA_ENC_DEC.KISA_ENCRYPT, pbszUserKey, pbszIV);
	...
	public static byte[] SEED_CBC_Decrypt(byte[] pbszUserKey, byte[] pbszIV, byte[] message, int message_offset,
		...
		KISA_SEED_CBC.Common.arraycopy(newpbszCipherText, pbszCipherText, nCipherTextLen);
		
		nCipherTextLen = newpbszCipherText.length;
		
		
		SEED_CBC_init(info, KISA_SEED_CBC.KISA_ENC_DEC.KISA_DECRYPT, pbszUserKey, pbszIV);
		...
		if(SEED_CBC_Close(info, outbuf, (nRetOutLeng[0]), nPaddingLeng) == 1) {
			...
			KISA_SEED_CBC.Common.arraycopy(pbszPlainText, cdata, nRetOutLeng[0] - nPaddingLeng[0]);
	...
	public static final class KISA_ENC_DEC {
		...
		public static final KISA_SEED_CBC.KISA_ENC_DEC KISA_ENCRYPT = new KISA_SEED_CBC.KISA_ENC_DEC(_KISA_ENCRYPT);
		public static final KISA_SEED_CBC.KISA_ENC_DEC KISA_DECRYPT = new KISA_SEED_CBC.KISA_ENC_DEC(_KISA_DECRYPT);
	...
	public static final class KISA_SEED_INFO {
		...
		public final KISA_SEED_CBC.KISA_SEED_KEY seed_key = new KISA_SEED_CBC.KISA_SEED_KEY();
	...
	public static class Common {
		...
		public static void arraycopy_offset(byte[] dst, int dst_offset, byte[] src, int src_offset, int length) {
			if(length >= 0) System.arraycopy(src, src_offset, dst, dst_offset, length);
		}
		...
		public static int Padding(byte[] pbData, byte[] padData, int length) {
			...
			KISA_SEED_CBC.Common.arraycopy(padData, pbData, length);
			i = length;
			do {
	...
	public static int SeedRoundKey(KISA_SEED_CBC.KISA_SEED_INFO pInfo, KISA_SEED_CBC.KISA_ENC_DEC enc,
								   byte[] pbszUserKey, byte[] pbszIV) {
		...
		KISA_SEED_CBC.Common.memcpy(pInfo.ivec, pbszIV, 16, ENDIAN);
		pInfo.last_block_flag = pInfo.buffer_length = 0;
		
		// Set up input values for Key Schedule
		ABCD[ABCD_A] = KISA_SEED_CBC.Common.byte_to_int(pbszUserKey, 0, ENDIAN);
		ABCD[ABCD_B] = KISA_SEED_CBC.Common.byte_to_int(pbszUserKey, 4, ENDIAN);
		ABCD[ABCD_C] = KISA_SEED_CBC.Common.byte_to_int(pbszUserKey, 2 * 4, ENDIAN);
		ABCD[ABCD_D] = KISA_SEED_CBC.Common.byte_to_int(pbszUserKey, 3 * 4, ENDIAN);
